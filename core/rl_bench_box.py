...
class RLBenchBox(TaskInterface):
    ...
    def send_movement(self, movement, duration):
        ...
        if self._headless:
            trajectory = movement.get_full_trajectory(duration=min(duration, 1), frequency=200)
        else:
            trajectory = movement.get_full_trajectory(duration=5 * duration, frequency=200)
        tot_reward = 0.
        success = 0
...
class Reacher2D(TaskInterface):
    ...
    def get_demonstrations(self, n: int):
        return np.array([np.concatenate(self._generate_demo(), axis=0) for _ in range(n)])

    def send_movement(self, movement, duration):
        duration = 1 if duration < 0 else duration
        trajectory = movement.get_full_trajectory(duration=duration, frequency=200)
